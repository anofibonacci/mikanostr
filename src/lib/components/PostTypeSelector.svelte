...
<div class="my-3">
	<select
		id="type"
		name="type"
		class="mt-1 block w-full rounded-md border-gray-900 py-2 pl-3 pr-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 text-black text-2xl"
		on:change={change}
	>
		...
		<div class="flex flex-col md:flex-row">
			<div class="my-3 flex-1 md:mr-1">
				...
				<input
					type="date"
					name="checkIn"
					class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-black text-2xl py-3"
				/>
			</div>
	...
	<div class="my-3">
		<label for="comment" class="block font-medium text-white">
			{#if visibleChoice === 'lodging'}
				<h3 class="text-lg font-semibold">
					Describe the arrangement a bit so nostriches booking know what to expect
				</h3>
			{:else}
				Add your comment
			{/if}
		...
		<div class="mt-1">
			<textarea
				...
				name="comment"
				id="comment"
				class="block w-full rounded-md border-gray-300 border shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 text-black"
				required
				minlength="10"
			/>
		</div>
