    ...
    <div class="flex flex-col">
        ...
        {#if content.price}
            <div class="flex flex-1 items-end">
                <div
                    class=" bg-slate-100 shadow border px-3 py-1 flex-shrink rounded-xl"
                >
                    <img
                        src="https://abs.twimg.com/hashflags/Bitcoin_evergreen/Bitcoin_evergreen.png" alt="Bitcoin"
...
<div class="flex-shrink px-4 flex flex-row items-center gap-1">
    <div class="flex-shrink flex flex-row items-center">
        ...
        {:else if content.checkIn}
            <div class="mr-2">
                <CalendarIcon
                    date={content.checkIn}
                    endDate={content.checkOut}
                />
            </div>
        {/if}
    </div>
    ...
    <div
        class="flex flex-col md:flex-row-reverse justify-center md:justify-start items-center"
    >
        ...
        {#if $nostrNotes.responses[post.id]}
            ...
            >
                <span class="font-bold text-md"
                    >{$nostrNotes.responses[post.id].length}</span
                >
                <span>
                    <svg
